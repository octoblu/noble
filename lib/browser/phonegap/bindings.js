  ...
  ble.discover(function(data){
    ...
    uuids.forEach(function(uuid){
      ...
      if(checkUuid.length > 7){
        checkUuid = checkUuid.substring(4,8).toLowerCase();
      }
      data.services.forEach(function(service){
        if(service.serviceAssignedNumber === checkUuid){
          matchingServices.push(service.serviceAssignedNumber);
        }
      });
    });
  ...
  ble.discover(function(data){
    ...
    data.services.forEach(function(service){
      ...
      if(checkServiceUuid.length > 7){
        checkServiceUuid = checkServiceUuid.substring(4,8).toLowerCase();
      }
      if(service.serviceAssignedNumber === checkServiceUuid){
        ...
        characteristicUuids.forEach(function(uuid){
          ...
          service.characteristics.forEach(function(characteristic){
            if(characteristic.characteristicAssignedNumber === checkUuid){
              matchingChars.push({uuid: characteristic.characteristicAssignedNumber});
            }
          });
        });
...
NobleBindings.prototype.read = function(deviceUuid, serviceUuid, characteristicUuid) {
  ...
  ble.read(function(resp){
    ...
    self.emit('read', deviceUuid, serviceUuid, characteristicUuid, ble.encodedStringToBytes(resp.value), false);
  }, function(err){
